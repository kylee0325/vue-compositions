...
function testUseEvent(description: string, targetFn: () => Target, isRef = false) {
  describe(description, () => {
    ...
    beforeEach(() => {
      ...
      wrapper = invokeHook(() => {
        onMounted(() => {
          ;[target, clear] = useEvent('click', handler, true, !isRef ? targetFn() : testRef)
        })
        return {
          test: testRef
        }
...
testUseEvent('test useEvent when target is an Element', () => document.getElementById('test')!)

testUseEvent('test useEvent when target is a css selector', () => '#test')

testUseEvent('test useEvent when target is a Ref', () => ref(null), true)
